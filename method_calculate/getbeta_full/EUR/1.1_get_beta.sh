#!/bin/bash
#SBATCH --job-name=beta_EUR_cal
#SBATCH --output=/gpfs/gibbs/pi/zhao/zb222/Anotation_pipeline/method_calculate/getbeta_full/Log/3.16/beta_EUR_cal_%A_%a.log
#SBATCH --requeue
...
IFS=',' read -r trait pop k chr param_phi < <(sed -n "${SLURM_ARRAY_TASK_ID}p" ${SCRIPT_DIR}/job_params.txt)
...
--sst_file=/gpfs/gibbs/pi/zhao/zb222/Data/summary_data/PRS-CSx/${trait}_${pop}/${chr}_JointPRS_baselineLD_v2.3_Annot_${k}.txt \
--n_gwas=${sample_size} \
--rho_cons=1,1,1,1,1 \
--chrom=${chr} \
--pop=${pop} \
--out_dir=$OUT_DIR/trait_${trait}/tar_${pop}/baselineLD_v2.3_Annot_${k} \
--out_name=${trait}_${pop}_JointPRS_baselineLD_v2.3_Annot_${k}
